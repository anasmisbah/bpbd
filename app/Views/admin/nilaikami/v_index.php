...
<?= $this->section('content-header'); ?>
    ...
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Nilai kami</li>
        </ol>
    </div><!-- /.col -->
<?= $this->endSection(); ?>
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Data Nilai Kami</h3>
        <div class="card-tools">
            <ul class="nav nav-pills ml-auto">
            <li class="nav-item">
                <a class="nav-link active" href="<?= route_to('nilaikami.create'); ?>"><i class="fas fa-plus"></i></a>
            </li>
            </ul>
        </div>
        </div>
        ...
        <table id="example2" class="table table-bordered table-striped">
            ...
            <tbody>
            <?php $i = 1; ?>
            <?php foreach($nilaikami as $item): ?>
            <tr>
                <td><?= $i++; ?></td>
                <td><?= $item['judul']; ?></td>
                <td><?= implode(' ', array_slice(explode(' ', $item['deskripsi']), 0, 6)).'...'; ?></td>
                <td>
                    <a href="<?= route_to('nilaikami.edit',$item['id']); ?>" class="btn btn-warning btn-sm">
                        <i class="fas fa-edit"></i>
                    </a>
                    <?php if($item['id'] != 1): ?>
                        <a href="javascript:;" data-action="<?= route_to('nilaikami.delete'); ?>" data-item_id="<?= $item['id']; ?>" class="btn btn-sm btn-danger btn-delete">
                            <i class="fas fa-trash"></i>
                        </a>
                    <?php endif; ?>
                    <a href="<?= route_to('nilaikami.detail',$item['id']); ?>" class="btn btn-info btn-sm">
